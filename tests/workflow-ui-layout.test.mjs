...
test('new workflow run starts from a drawer with git safety confirmation', () => {
  ...
  assert.match(drawer, /Workflow Template/)
...
test('main navigation is consolidated to workflow templates agents single and settings', () => {
  ...
  assert.match(app, /case 'settings':\s*return 'Settings'/)
  assert.match(templatesView, /templates-sidebar-head/)
  assert.match(templatesView, /<h2>Templates<\/h2>/)
  assert.match(modeRail, /railIcon\('workflow'\)[\s\S]*railIcon\('templates'\)[\s\S]*railIcon\('agents'\)[\s\S]*railIcon\('single'\)[\s\S]*railIcon\('settings'\)/)
  assert.match(modeRail, /<span>Workflow<\/span>[\s\S]*<span>Templates<\/span>[\s\S]*<span>Agents<\/span>[\s\S]*<span>Single<\/span>[\s\S]*<span>Settings<\/span>/)
...
test('ui review chrome matches v4 mockup navigation and composer', () => {
  ...
  assert.match(modeRail, /Workflow[\s\S]*Layers[\s\S]*Bot[\s\S]*Play[\s\S]*Settings/)
  ...
  assert.match(css, /\.workflow-cli-composer \.composer-row\s*\{[\s\S]*display:\s*grid;[\s\S]*grid-template-columns:\s*32px minmax\(0,\s*1fr\) 32px;[\s\S]*align-items:\s*center;/)
