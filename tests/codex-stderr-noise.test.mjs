...
const filterCodexStderr = extractFunction(adapterSource, 'filterCodexStderr')
...
assert.equal(
  ...
  '',
  'Codex plugin asset icon warnings should be removed from stderr'
)

assert.equal(
  filterCodexStderr(pluginIconLargeWarning),
  '',
  'Codex plugin large-icon asset warnings should also be removed from stderr'
)
...
assert.equal(
  ...
  'fatal: not a git repository',
  'real stderr mixed with noise should still be visible'
)

assert.equal(
  filterCodexStderr('fatal: not a git repository'),
  'fatal: not a git repository',
