...
test('shared contract exposes schedules, scheduled runs, app setting, and IPC channels', () => {
  ...
  assert.match(types, /type WorkflowScheduleTargetType = 'workflow' \| 'agent'/)
  assert.match(types, /targetType: WorkflowScheduleTargetType/)
  assert.match(types, /templateId\?: string/)
  assert.match(types, /agentId\?: string/)
  assert.match(types, /cron: string/)
  assert.match(types, /interface WorkflowSchedule \{[\s\S]*useWorktree\?: boolean/)
  ...
  assert.match(types, /templateSnapshot\?: WorkflowTemplate/)
...
test('schedule store persists schedules and last trigger metadata', () => {
  ...
  assert.match(store, /targetType/)
  assert.match(store, /agentId/)
  assert.match(store, /Workflow template is required/)
  assert.match(store, /Agent is required/)
...
test('scheduler registers enabled schedules, starts auto-confirm runs, and updates terminal status', () => {
  ...
  assert.match(scheduler, /useWorktree: schedule\.useWorktree \?\? true/)
  assert.match(scheduler, /schedule\.targetType === 'agent'/)
  assert.match(scheduler, /agentId/)
  assert.match(scheduler, /startAdHoc/)
...
test('workflow manager records scheduled metadata and auto-confirms successful handoffs', () => {
  ...
  assert.match(manager, /startAdHoc/)
  assert.match(manager, /templateSnapshot/)
  assert.match(manager, /run\.templateSnapshot/)
...
test('renderer exposes schedules tab, drawer, detail history, settings toggle, and scheduled run badge', () => {
  ...
  assert.match(scheduleWorkspace, /agents: AgentDefinition\[\]/)
  ...
  assert.match(scheduleList, /AgentDefinition/)
  assert.match(scheduleList, /scheduleTargetName/)
  assert.match(scheduleList, /toggle/)
  assert.match(scheduleList, /cronDescribe/)
  assert.match(scheduleDetail, /scheduledBy/)
  assert.match(scheduleDetail, /scheduleTargetName/)
  assert.match(scheduleDetail, /onOpenRun/)
  assert.match(scheduleDrawer, /targetType/)
  assert.match(scheduleDrawer, /selectedAgent/)
  assert.match(scheduleDrawer, /const \[useWorktree, setUseWorktree\] = useState\(schedule\?\.useWorktree \?\? true\)/)
  assert.match(scheduleDrawer, /useWorktree/)
  assert.match(scheduleDrawer, /setUseWorktree/)
  assert.match(scheduleDrawer, /在隔离副本中运行/)
...
test('schedule drawer uses n8n-style interval picker instead of requiring cron first', () => {
  ...
  assert.match(scheduleDrawer, /Project Directory/)
  assert.match(scheduleDrawer, /value=\{projectPath\}/)
  assert.match(scheduleDrawer, /onChange=\{\(event\) => setProjectPath\(event\.target\.value\)\}/)
  assert.match(scheduleDrawer, /FolderOpen/)
  assert.match(scheduleDrawer, /aria-label="Browse project directory"/)
  assert.match(scheduleDrawer, /window\.api\.pickDir/)
  assert.match(scheduleDrawer, /checked=\{useWorktree\}/)
  assert.match(scheduleDrawer, /useWorktree,\s*\n\s*createdAt/)
  assert.match(scheduleDrawer, /Trigger Interval/)
  assert.match(scheduleDrawer, /scheduleCronStateFromPreset/)
  assert.match(scheduleDrawer, /function ScheduleTimeSelect/)
  assert.match(scheduleDrawer, /className="schedule-time-select"/)
  assert.match(scheduleDrawer, /ariaLabel="Select hour"/)
  assert.match(scheduleDrawer, /ariaLabel="Select minute"/)
  assert.match(scheduleDrawer, /contentClassName="schedule-time-select-content"/)
  assert.doesNotMatch(scheduleDrawer, /type="time"/)
  ...
  assert.match(styles, /\.schedule-time-select\s*\{/)
  assert.match(styles, /\.schedule-time-separator\s*\{/)
  assert.match(styles, /\.schedule-time-select-content\s*\{/)
})
