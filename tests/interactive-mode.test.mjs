...
test('renderer shows awaiting-input status and enables replies', () => {
  ...
  assert.match(runsList, /case 'awaiting-input':\s*return \{ label: '待回复'/)
  ...
  assert.match(detail, /const showStepConversationBar = awaitingInput && !!selectedExecution\?\.conversation/)
  assert.match(detail, /\{showStepConversationBar && \(/)
  assert.doesNotMatch(detail, /\{selectedExecution\?\.conversation && \(/)
