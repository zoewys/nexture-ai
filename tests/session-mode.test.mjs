...
test('single route controls render as a compact toolbar instead of large cards', () => {
  ...
  const codexAdvancedToolbarBlock = block(/\.single-session-toolbar-main-codex-advanced\s*\{([^}]*)\}/)
  ...
  assert.match(toolbarBlock, /grid-template-columns:\s*minmax\(260px,\s*320px\) minmax\(320px,\s*1fr\) minmax\(560px,\s*720px\) !important;/)
  assert.doesNotMatch(toolbarBlock, /1\.45fr/)
  assert.match(toolbarBlock, /align-items:\s*start !important;/)
  assert.match(codexAdvancedToolbarBlock, /grid-template-columns:\s*minmax\(220px,\s*300px\) minmax\(280px,\s*1fr\) minmax\(430px,\s*540px\) minmax\(360px,\s*440px\) !important;/)
  ...
  assert.match(inlineAdvancedBlock, /justify-self:\s*end !important;/)
  assert.match(inlineAdvancedBlock, /align-self:\s*start !important;/)
